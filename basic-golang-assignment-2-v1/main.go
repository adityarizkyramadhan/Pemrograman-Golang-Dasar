...
func Login(id string, name string) string {
	...
	for _, student := range Students {
		dataOfStudent := strings.Split(student, "_")
		if dataOfStudent[0] == id && dataOfStudent[1] == name {
			return fmt.Sprintf("Login berhasil: %s", dataOfStudent[1])
		}
	}
...
func UpdateStudyProgram(programStudi string, students *string) {
	dataOfStudent := strings.Split(*students, "_")
	*students = fmt.Sprintf("%s_%s_%s", dataOfStudent[0], dataOfStudent[1], programStudi)
}
